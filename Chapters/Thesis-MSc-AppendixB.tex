...
This appendix presents some useful results and techniques which can be used to increase its accuracy. Firstly, we present the behavior of the Laplace equation's solutions in polar coordinates near a corner's tip. Such considerations were the basis for the results presented in \ref{numerical_transmission_simulations}. Then, the Subspace Angle Technique is also presented, which was used in \ref{dirac_equation_simulations} to diminish the ill-conditioning of the \ac{MFS} for the Dirac equation with infinite mass boundary conditions.

\section{Behavior of the Laplace equation's solutions near a corner.}
...
\section{The Subspace Angle Technique}
