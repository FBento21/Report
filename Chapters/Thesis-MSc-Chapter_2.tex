...
Motivated by recent advancements in nuclear, and molecular physics and the discovery of very interesting electrical, mechanical, and thermal properties, Dirac materials have resurged a lot of attention in the Dirac equation. Presented by Paul Dirac in its 1928 article \cite{dirac1928quantum}, Dirac equation was able to successfully merge the famous Schr\"{o}dinger equation with special relativity, while explaining the phenomena that today is known as \textit{spin}, while predicting the existence \textit{antimatter}.
Capable of describing the relativistic dynamics of spin-$\frac{1}{2}$ particles (like the electron), one can determine its energy states by studying the spectrum of the Hamiltonian (Dirac) operator \(\hat{H}\) in \(L^2(\Omega, \mathbb{C}^2)\) for \(\Omega \subset \mathbb{R}^2\),
...
\begin{proposition}\label{dirac_properties}
    ...
    \begin{itemize}
        ...
        \end{pmatrix}.    
        \]
        For future comparison, its numerical approximation is \(\lambda_1 \approx 1.434695650819\), where \(J_p\) is the Bessel function of the first kind of order \(p\).
    \end{itemize}
\end{proposition}

The following proposition regarding the lack of separable solutions of the Dirac operator will have interesting consequences in the numerical approach to solve the Dirac equation. However, we start by stating and proving the following auxiliary lemma:

\begin{lemma}\label{lemma_sep_norm}
    Let \(u \in H^2(\Omega)\) a solution of \eqref{dirac} and \(u \in D(\hat{H})\). Then,
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item For \eqref{sep_norm_4}, firstly the following property was deduced:
                ...
                \end{align*}
                where the property above was used.
    \end{itemize}
    ...
    \end{align*}
    where the Frenet-Serret formula above was used and the fact that on \(\Gamma\) one has \(\abs{u_1}^2=\abs{u_2}^2\).
\end{proof}
...
\begin{proof}
    ...
    \begin{equation}\label{helm_system}
        ...
        \end{cases}      
    \end{equation}
    The rest of the proof will have into consideration two domain types: first, for polar coordinates, one studies the behavior of such separable solution near a corner, such that the origin is in the corner's tip; then, rectangular domains with cartesian coordinates.
    ...
    \]
    From the recurrence relations for the derivatives of Bessel functions\footnote{Once again, we point the reader to Appendix \ref{appendix_c_bessel}.}
    \[
...
Some open conjectures that we try to numerically address in this work are now presented.
...
In this work, we generalize the conjectures above for the \(n\)-side polygons with unit area, analogous to the Pólya-Szég\H{o} Conjecture \ref{polya_szego_conjecture}.

\begin{conjecture}\label{polya_szego_conjecture_dirac}
    Let \(m \geq 0\), \(\Omega \subset \mathbb{R}^2\) an open and Lipschitz domain, \(n \geq 5\) and consider the class of \(n\)-side polygons. Then, the regular \(n\)-side polygon has the least first eigenvalue among all \(n\)-side polygons with fixed area.
\end{conjecture}
...
\begin{align}\label{decomp_prob}
    ...
    \end{cases}
\end{align}
where \(k_1 \geq k_2 > 0\) are constants, \(f_i \in L^2(\Omega_i)\) is a source function on each domain, and \(n_i\) is the (normalized) outward normal to each domain subdomain \(\Omega_i, i=1, 2\). Finally, we shall write \(n=n_1=-n_2\) when we are restricted to the interface.
...
\begin{align}\label{decomp_poisson}
    ...
    \end{cases}
\end{align}
where \(f\) is (possibly) discontinuous on the interface \(\gamma\). In order to keep the equivalence between both problems, we enforce some transmission conditions in Equations \eqref{decomp_prob} using the continuity of the solutions and the continuity of their normal derivative on \(\gamma\). To establish the equivalence between both problems we write the variational weak form associated with them. For \eqref{decomp_poisson} it is straightforward: one multiplies the equation in \(\Omega\) by a test function \(v \in C^\infty_0(\Omega)\) and by Green's Identities one finds that
\[
a(u,v)=\int_\Omega \nabla u \cdot \nabla v = \int_\Omega f v
\]
where \(a\) is the associated bilinear form. As such, enlarging the functional space to the Hilbert Space \(V^0=H^1_0(\Omega)\), the problem can be rewritten as
\begin{equation}\label{weak_poisson}
    \text{find } u \in V^0: a(u,v) = (f, v), \; \forall v \in V^0   
\end{equation}
which has a unique solution in \(V^0\) by virtue of Lax-Milgram lemma \ref{lax-milgram}. In fact, by regularity results\footnote{We write that \(A \Subset B\) when \(\overline{A}\) is compact and \(\overline{A} \subset B\).}  (see Section 6.3.1 of \cite{evans2022partial}), \(u \in V^0 \cap H^2_{\text{loc}}(\Omega'), \; \forall \Omega' \Subset \Omega\). 

For \eqref{decomp_prob} the process is not so direct. Given the subdomain \(\Omega_1\), one multiplies the first equation with a test function in \(v_1 \in C^\infty_0(\Omega_1)\) and integrate by parts,
\[
...
\begin{remark}
    The existence of the (continuous) extension operators \(P_1\) and \(P_2\) is not obvious but Theorem \ref{frac_theo} states the existence of a right inverse of the trace operator. See the presented references for more details. 
\end{remark}
...
\begin{proof}
    ...
    Finally, given \(\mu \in \Lambda\), one defines the function
    \[
        P \mu =
    \begin{cases}
        P_1 \mu\\
    ...
    Let \(u_1 \in V_1^0, u_2 \in V_2^0\) be the two solutions of problem \eqref{weak_poisson} and set
    \[
    u=\begin{cases}
        u_1, \text{ in } \Omega_1\\
        u_2, \text{ in } \Omega_2.
    ...
    Defining \(P \mu\) like above, take \(\mu \in \Lambda\) for some \(v \in V\) such that \(\mu = v_{|\gamma}\). As such, one has that the difference \((v_{|\Omega_i} - P_i \mu) \in V^0_i\) for each \(i=1, 2\) and
    \begin{align*}
