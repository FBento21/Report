...
\begin{enumerate}
    \item Dirac
    \begin{enumerate}
        \item Result against the ball with m=0 (accuracy): %We need image -> L^2 quadrature but this is easy
        \item Results for quadrilaterals and Conjecture: We need image %-> L^2 quadrature
        \item Results for triangles and Conjecture
        \item Results for smooth domains 
        \item Third eigenvalue method: We need image %-> L^2 quadrature
    \end{enumerate}
    \item Transmission
    \begin{enumerate}
        \item (WE STILL NEED PROOF FOR DECOMPOSITION DENSITY!)
...
As said above, the BFGS method was also used. However, no meaningful results were found using this method, since any descent direction produced not  
