...
As the name implies, \ac{MFS} is based on the fundamental solutions of a previously known \ac{PDE}. Consider the Elliptic linear differential operator \(\mathcal{L}\) with fundamental solution \(\Phi\) such that \(\mathcal{L}\Phi(x) = \delta, \; \forall x \in \mathbb{R}^d\). Intuitively, we can consider the approximation
\[
...
\end{theorem}
\begin{proof}
    Given Lemma \ref{banach_ortho_lemma} we start by fixing some notation. Let \(E = H^\frac{1}{2}(\Gamma)\). For every (fixed) \(y \in \hat{\Gamma}\), the maps
    \begin{align*}
...
The density proof for the transmission/decomposition problem follows directly from the Theorem above and the equivalence formulation presented in Theorem \ref{equivalence_transmission}. Consider the source function
\begin{equation}
    f = \begin{cases}
...
\begin{itemize}
    ...
    \item if \(M > N+1\), then we must solve a least-squares problem. Observe that the case \(M < N+1\) is an under-determined system of equations and therefore the number of colocation points must be greater than the number of source points. In any case, every numerical linear algebra software can solve this type of problem efficiently. This is the method used in this work, since it avoids interpolation instabilities, forces the boundary condition to hold at some specific points, and is particularly robust when dealing with non-regular boundary data.  After several numerical experiments, e.g. \cite{alves2009choice}, it was concluded that \(N=2M\) is a good choice for the number of source points.
\end{itemize}
...
\]
where \(c_1, c_2, c_3, c_4 \in \mathbb{C}\). In order to find \(\alpha\), one must consider the amplitude of the angle \(\Theta\) and the boundary conditions at each segment \(\overrightarrow{OA}\) and \(\overrightarrow{OB}\). We summarize the asymptotic harmonic solutions of \eqref{lap_polar} in Appendix \ref{chapter:appendixB}. 
% \begin{itemize}
